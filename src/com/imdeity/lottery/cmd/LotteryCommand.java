...
public class LotteryCommand implements CommandExecutor {
	...
	public void buyCommand(Player player, String[] split)
			throws InvalidFundsException, NegativeMoneyException,
		...
		sql = "INSERT INTO  "
				...
				+ "');";
		double money = numTicket
				* Lottery.config.getInt("lottery.ticket_price");
		...
		if (numTicket < 1) {
			Deity.chat.sendPlayerError(player,
					"Negative Tickets? Yea...that makes sense.");
			return;
		}

		if ((LotteryObject.getNumTickets(player.getName()) + numTicket) > Lottery.config
				.getInt("lottery.max_tickets")) {
			Deity.chat.sendPlayerError(
					player,
					"You can only have a maximum of "
							+ Lottery.config.getInt("lottery.max_tickets")
							+ " Tickets.");
			return;
		}
