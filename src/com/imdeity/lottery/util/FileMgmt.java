...
public class FileMgmt {
    ...
    public static void moveFile(File sourceFile, String targetLocation)
        ...
        if (sourceFile.isFile()) {
            ...
            File f = new File(
                    (sourceFile.getParent() + fileSeparator() + targetLocation));
            if ((f.exists() && f.isFile()))
                f.delete();
            // Move file to new directory
            boolean success = sourceFile.renameTo(new File((sourceFile
                    .getParent() + fileSeparator() + targetLocation),
                    sourceFile.getName()));
            if (!success) {
    ...
    public static void zipDirectory(File sourceFolder, File destination)
            throws IOException {
    ...
    public static void zipDirectories(File[] sourceFolders, File destination)
            throws IOException {
        ZipOutputStream output = new ZipOutputStream(new FileOutputStream(
                destination));
