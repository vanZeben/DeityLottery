...
public class LotteryObject {
    ...
    public static int getPot() {
        String sql = "SELECT * FROM "+Settings.getMySQLPlayersTable()+" ORDER BY `id` DESC LIMIT 1";  
        
        try {
            return ((Integer.parseInt(Lottery.database.Read(sql).get(1).get(0))) * (Settings.getTicketPrice()))
    ...
    public static String drawWinner() {
      ...
      try {
          winnings = getPot();
          sql = "SELECT * FROM "+Settings.getMySQLPlayersTable()+" ORDER BY RAND() LIMIT 1";
    ...
    public static ArrayList<String> getWinners() {
        ...
        String sql = "";
        sql = "SELECT * FROM "+Settings.getMySQLWinnersTable()+" ORDER BY `id` DESC LIMIT 10";  
       
        HashMap<Integer, ArrayList<String>> winners= Lottery.database.Read(sql);
        for (int i = 1; i <= winners.size(); i++) {
            out.add(ChatTools.formatUserList(winners.get(i).get(1), winners.get(i).get(2)));
        }
        return out;
    }
    ...
    public static int getNumTickets(String name) {
        String sql = "SELECT * FROM "+Settings.getMySQLPlayersTable()+" WHERE `username` = '"+name+"';";  
        try {
