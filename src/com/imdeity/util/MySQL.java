...
public class MySQL {
	...
	public MySQL() {
		...
		try {
        	...
        	throw new DataSourceException(plugin+" Failed to initialize JDBC driver");
        }
		...
		try {
			...
			dumpSqlException(ex);
			throw new DataSourceException(plugin+" Failed to create connection to Mysql database");
		}
	...
	private void reconnect() {
		...
		try {
			...
			System.out.println(plugin+" Connection to MySQL failed! Check status of MySQL server!");
			dumpSqlException(ex);
		}
	}
		...
		PreparedStatement stmt = conn.prepareStatement(sql);
  		...
  		for (Object param:params) {
  			if (param instanceof Integer) {
  				...
  				System.out.printf(plugin+" Database -> Unsupported data type %s", param.getClass().getSimpleName());
  			}
  		}
  		return stmt;
	}
	...
	private String getConnectionString() {
		return "jdbc:mysql://" 
		+ Settings.getMySQLServerAddress() + ":" 
		+ Settings.getMySQLServerPort() + "/" 
		+ Settings.getMySQLDatabaseName() 
		+ "?user=" + Settings.getMySQLUsername()
	...
	public static String tableName(String nameOfTable) {
		return( String.format("`%s`.`%s`", Settings.getMySQLDatabaseName() , Settings.getMySQLDatabaseTablePrefix() + nameOfTable));
	}
